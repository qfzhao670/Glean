...
- **YouTube / 哔哩哔哩 → 笔记**：使用 yt-dlp 提取已有人工/自动字幕；自动字幕优先使用原始语言，由模型生成中文笔记，避免依赖平台自动翻译字幕。无字幕直接停止，不下载视频转写。保留原始字幕用于查阅、导出和对话。
...
4. 从「拾取新知」导入内容。点击顶部进度状态查看当前任务和失败原因；完成后在「我的笔记」查看结果，进度面板不展示历史记录。
