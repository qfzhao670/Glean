...
import { ArrowUpRight, BookOpen, Check, ChevronRight, FileText, FileVideo, Leaf, Link2, LoaderCircle, Plus, Settings2, Sparkles, Sprout, X } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="window-space"/><button className="brand" onClick={() => navigate('home')}><Logo/><span>拾知<small>Glean</small></span></button><div className="sidebar-caption">在这里，知识慢慢生长</div><button className="new-note-button" onClick={() => setModal('txt')}><Plus size={18}/>拾取新知<span>⌘ N</span></button><nav><button className={page === 'home' ? 'active' : ''} onClick={() => navigate('home')}><Sprout size={19}/>知识花园<ChevronRight size={14}/></button><button className={page === 'notes' ? 'active' : ''} onClick={() => navigate('notes')}><BookOpen size={19}/>我的笔记<span className="nav-count">{notes.length.toString().padStart(2, '0')}</span></button></nav>
    <div className="sidebar-bottom"><div className="sidebar-quote"><Leaf size={21} strokeWidth={1.1}/><p>不必急着成为森林。<br/>今天，长出一片新叶就好。</p><span>一点一滴，皆有所获。</span></div><button className={`sidebar-settings ${page === 'settings' ? 'active' : ''}`} onClick={() => navigate('settings')}><Settings2 size={18}/>偏好设置</button><button className="vault-indicator" onClick={() => navigate('settings')}><span className={`status-dot ${settings?.repository_path ? '' : 'neutral'}`}/><span>{settings?.repository_path ? '本地笔记仓库' : '选择本地笔记仓库'}<small>{settings?.repository_path ? settings.repository_path.split(/[\\/]/).filter(Boolean).pop() : '让知识回到你自己的地方'}</small></span><ArrowUpRight size={15}/></button></div></aside>
    ...
    {page === 'notes' && !selected && <Library notes={notes} onOpen={openNote} onGenerate={() => setModal('txt')} onChanged={refresh} notify={notify}/>}
    {page === 'notes' && selected && <NoteDetail key={selected} id={selected} initialEdit={editNew} onDirtyChange={setDirty} onBack={() => navigate('notes')} onUpdated={refresh} onJobCreated={watchJob} notify={notify} onLink={title => { const target = notes.find(n => n.title === title); if (target) openNote(target.id); else notify('这篇关联笔记尚未导入，可以在「我的笔记」导入 Markdown'); }}/>}
